...
function start_agent() {
    ...
    sudo ${VERTICA_OPT_DIR}/sbin/vertica_agent start \
        2> ${VERTICA_DATA_DIR}/${DBNAME}/agent_start.err \
        1> ${VERTICA_DATA_DIR}/${DBNAME}/agent_start.out
}
...
function stop_agent() {
    ...
    sudo ${VERTICA_OPT_DIR}/sbin/vertica_agent stop \
        2> ${VERTICA_DATA_DIR}/${DBNAME}/agent_stop.err \
        1> ${VERTICA_DATA_DIR}/${DBNAME}/agent_stop.out
}
...
function vertica_proper_shutdown() {
    ...
    case "$db"x in
        ...
        *)
            ...
            ${ADMINTOOLS} -t stop_db -d ${DBNAME} -i
            ;;
    esac
}
...
echo 'Starting up'
if [ ! -d ${VERTICA_DATA_DIR}/${DBNAME} ]; then
...
echo "Creating schema '${DBSCHEMA}' in database '${DBNAME}'..."
${VSQL} -U dbadmin -w ${APP_DB_PASSWORD} -d ${DBNAME} -c "CREATE SCHEMA IF NOT EXISTS ${DBSCHEMA};"

# Create user '${APP_DB_USER}' with password 'admin1234'
echo "Creating user '${APP_DB_USER}'..."
${VSQL} -U dbadmin -w ${APP_DB_PASSWORD} -d ${DBNAME} -c "CREATE USER ${APP_DB_USER} IDENTIFIED BY '${APP_DB_PASSWORD}';"

# Grant all privileges to the user '${APP_DB_USER}' on schema '${DBSCHEMA}'
echo "Granting privileges to user '${APP_DB_USER}' on schema '${DBSCHEMA}'..."
${VSQL} -U dbadmin -w ${APP_DB_PASSWORD} -d ${DBNAME} -c "GRANT ALL ON SCHEMA ${DBSCHEMA} TO ${APP_DB_USER};"
